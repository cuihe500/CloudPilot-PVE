...
- AI 使用独立机器账号推送和创建 PR；项目 Owner `@cuihe500` 不作为 PR 作者，否则 GitHub 无法将其加入 Reviewers。
...
- Reviewers 必须包含 `@cuihe500`，并由其完成项目 Owner 审查。
- `.github/CODEOWNERS` 为所有路径指定 `@cuihe500`，分支保护启用后不得绕过其批准。
- 评审修正在同一分支追加提交。
- 每次影响行为的修正都重新运行相关检查。
- 新增重要修改时同步更新 PR 正文，而不是只在评论中说明。
- `@cuihe500` 批准、CI 通过且评审对话全部解决后才允许合并。
