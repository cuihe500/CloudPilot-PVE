...
validate_body() {
  ...
  ! grep -Fq '<!--' "$body_file" || fail "PR body contains unresolved template comments"
...
create_pr() {
  ...
  if [[ "$draft" == "true" ]]; then
    pulls="$(GH_TOKEN="$token" "$GH" api graphql \
      -f query='mutation($id:ID!){markPullRequestReadyForReview(input:{pullRequestId:$id}){pullRequest{number isDraft}}}' \
      -F id="$node_id")"
    jq -e '.data.markPullRequestReadyForReview.pullRequest.isDraft == false' <<<"$pulls" >/dev/null || fail "PR #$number was not confirmed ready for review"
  fi
